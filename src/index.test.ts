  ...
  test("seeding", (done) => {
    ...
    _connection((ws) => {
      ...
      masterWire.on("seeding", (info: any) => {
        expect(nodeHandshake).toHaveBeenCalled()
        expect(masterHandshake).toHaveBeenCalled()
        expect(info.infoHashes).toEqual(expect.arrayContaining(seedingInfoHashes))
        expect(info.timestamp).toBeLessThanOrEqual(Date.now())
        expect(info.timestamp).toBeGreaterThanOrEqual(startTimestamp)
        _closeAll(done)
      })
      ...
      nodeWire.handshake()
        .then(() => {
          ...
          nodeWire.on("warning", (info: any) => {
            ...
            expect(info.message).toBe(Wire.WARNING_MSG[ Wire.WARNING_MSG_ID.OLD_NODE_VERSION ])
            expect(info.timestamp).toBeGreaterThanOrEqual(startTimestamp)
...
describe("handshake validation", () => {
  test("should fail node signature check", (done) => {
    ...
    _connection((ws) => {
      const masterWire = new Wire(ws, msgToNode, msgSignedToNode, function signCheck(msgFrom: any, msgSignedFrom: any) {
    ...
    _listen(() => {
      const nodeWire = new Wire(masterAddress, msgToMaster, msgSignedToMaster, function signCheck(msgFrom: any, msgSignedFrom: any) {
        return new Promise((resolve, reject) => {
  ...
  test("should pass signatures", (done) => {
    ...
    const nodeHandshakeFailed = jest.fn()
    _connection((ws) => {
      const masterWire = new Wire(ws, msgToNode, msgSignedToNode, function signCheck(msgFrom: any, msgSignedFrom: any, address?: string, ) {
        return new Promise((resolve, reject) => {
          resolve(msgFrom === msgSignedFrom * 2)
        })
      })
  ...
  test("should pass signatures", (done) => {
    ...
    const nodeHandshakeFailed = jest.fn()
    _connection((ws) => {
      const masterWire = new Wire(ws, msgToNode, msgSignedToNode, function signCheck(msgFrom: any, msgSignedFrom: any, address: any, version: string) {
        return new Promise((resolve, reject) => {
